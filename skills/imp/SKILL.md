...
For `TICKET`, work only in the reported worktree. Inspect any reported dirty state before editing. If the branch is behind the default branch, follow the repository's synchronization workflow before running `/implement` there. `/implement` owns the rest: it carries the ticket through checks, a pull request, and review until the work merges. Run `npx @limchihi/harness state` when the branch, remote, or pull-request relationship needs orientation.
