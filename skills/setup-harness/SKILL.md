---
name: setup-harness
description: "Configure this repo for the harness delivery loop — merge automation and worktree cleanup. Run once before the first /implement."
disable-model-invocation: true
---

# Set up harness

`/implement` carries a ticket through review to merge, and it leans on two
things every repository answers differently:
...
- `.agents/hooks/harness/` — are the hooks installed? If not, the user runs
  `npx @limchihi/harness install` before this configuration means anything.
...
found no per-worktree resources — `npx @limchihi/harness cleanup` already
removes the worktree and its branch, and a repository that owns nothing else
needs no hook.
...
Write it as an executable `.agents/hooks/cleanup`, using
[`cleanup`](./cleanup) as the starting point. `cleanup` runs it after removing a
merged worktree and reports its output, so have the command name what it
released rather than working silently.
...
Tell the user which files now carry the configuration, and that `/implement`
reads none of them directly — it reaches them through `npx @limchihi/harness
await` and `npx @limchihi/harness cleanup`. Re-running this skill is only
necessary to change how the repository merges or what it releases.
